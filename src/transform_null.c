...
static inline int transop_decode_null(n2n_trans_op_t *arg,
                                      uint8_t *outbuf,
                                      size_t out_len,
                                      const uint8_t *inbuf,
                                      size_t in_len,
                                      const uint8_t *peer_mac) {
    (void)arg;          // unused
    (void)peer_mac;     // unused

    if (out_len < in_len) {
#if TRACE_ENABLED && (TRACE_LEVEL >= TRACE_DEBUG)
        traceEvent(TRACE_DEBUG, "decode_null %zu too big for packet buffer", in_len);
#endif
        return -1;
    }

#if TRACE_ENABLED && (TRACE_LEVEL >= TRACE_DEBUG)
    traceEvent(TRACE_DEBUG, "decode_null %zu", in_len);
#endif
